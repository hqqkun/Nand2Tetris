...
public class JackTokenizer {
    ...
    private static String DecRegex;         // integer constant
    private static String HexRegex;
    ...
    private static String labelRegex;       // label, used when goto statement. Label:
    // keywordMap
    ...
    * */
    ...
    public int intVal() {
        if (currentTokenType == Type.INT_CONST_DEC) {
            return Integer.parseInt(currentToken);
        } else {
            // hex
            return Integer.parseInt(currentToken, 16);
        }
    }
    ...
    private static void initKeywordMap() {
        ...
        keywordMap.put("goto", Keyword.GOTO);
    ...
    private static void initRegexes() {
        ...
        // integer constant, also support Hexadecimal.
        DecRegex = "(0|([1-9]\\d*))";
        HexRegex = "0[Xx](0|[1-9a-fA-F][\\da-fA-F]*)";
        idRegex = "[A-Za-z_]\\w*";                      // identifier
        stringRegex = "\"[^\"\n]*\"";                   // string constant
        symbolRegex = "[{}()\\[\\].,;+\\-*/&|<>=~]";    // single char symbol
        labelRegex = "[A-Za-z_]\\w*:";                  // label
    ...
    private Type findTypeFromName(String tokenName) {
        if (tokenName.matches(keywordRegex)) {
            ...
            return Type.SYMBOL;
        } else if (tokenName.matches(HexRegex)) {
            return Type.INT_CONST_HEX;
        } else if (tokenName.matches(DecRegex)) {
            return Type.INT_CONST_DEC;
            ...
            return Type.NONE;
        } else if (tokenName.matches(labelRegex)) {
            return Type.LABEL;
        } else {
            return Type.ERROR;
        }
